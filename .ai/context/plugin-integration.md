...
> **Committed-state rule (enforced by `inkbridge-guard.mjs`):** the
> `inkbridge` dependency in `package.json` on any branch that gets pushed must
> resolve to a published npm version (`inkbridge@beta` or a pinned
> `^0.1.0-beta.X`), **never** `file:../inkbridge/tools/figma-plugin`. A
> `file:..` dep makes `pnpm install` fail for anyone who clones the starter
> without the sibling repo at that exact path.
>
> The rule is checked automatically — `.husky/pre-push` runs
> [inkbridge-guard.mjs](../../inkbridge-guard.mjs) which reads HEAD's
> `package.json` (not the working tree) and refuses the push if the dep starts
> with `file:` or `link:`. Friendly error tells you exactly which command to
> run. Working-tree-only local links are fine — the guard only blocks pushes,
> so you can iterate locally without flipping back.
